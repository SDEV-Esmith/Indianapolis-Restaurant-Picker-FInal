...
class Application:
    ...
    def handle_restaurant_and_image(self, chosen_restaurant, price_tier):
        ...
        if self.current_price_tier == "low": #If current price tier is low
            image = self.low_price.get(chosen_restaurant) #
        elif self.current_price_tier == "medium":
            image = self.medium_price.get(chosen_restaurant)
        else:
            image = self.high_price.get(chosen_restaurant)
    ...
    def try_another(self):
        ...
        #Store last restaurant to compare with new one
        comp_restaurant = self.last_restaurant

        #Get another restaurant and image
        if self.current_price_tier == "low": #If low price tier
            chosen_restaurant = rd.choice(list(self.low_price.keys())) #
            while chosen_restaurant == comp_restaurant:
                chosen_restaurant = rd.choice(list(self.low_price.keys()))
            image = self.low_price.get(chosen_restaurant)

        elif self.current_price_tier == "medium":
            chosen_restaurant = rd.choice(list(self.medium_price.keys()))
            while chosen_restaurant == comp_restaurant:
                chosen_restaurant = rd.choice(list(self.medium_price.keys()))
            image = self.medium_price.get(chosen_restaurant)

        else:
            chosen_restaurant = rd.choice(list(self.high_price.keys()))
            while chosen_restaurant == comp_restaurant:
                chosen_restaurant = rd.choice(list(self.high_price.keys()))
            image = self.high_price.get(chosen_restaurant)
